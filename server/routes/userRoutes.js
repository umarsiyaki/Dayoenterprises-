...
const userRoutes = {
  ...
  router.post('/security-question', userController.addSecurityQuestion);
...
router.post('/track-device', async (req, res) => {
    const { userId, ip, device } = req.body;
    const deviceRegistered = await deviceTrackingService.checkDevice(userId, ip, device);
    if (!deviceRegistered) {
        otpService.generateOTP(userId);
        return res.status(401).json({ message: 'New device detected, OTP required.' });
    }
    res.status(200).json({ message: 'Device recognized, access granted.' });
});
